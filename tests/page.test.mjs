...
test('services section lists at least three named treatments', async () => {
  ...
  assert.match(services, /The Stillness Facial/);
});
...
test('tokens are semantic — no raw hex colors in components', async () => {
  ...
  for (const f of await readdir(dir)) {
    ...
    assert.equal(hex, null, `${f} uses raw hex: ${hex}`);
  }
});

test('reduced motion and responsive breakpoints are handled', async () => {
  const css = await read('../app/globals.css');
  assert.match(css, /prefers-reduced-motion: reduce/);
  const tw = await read('../tailwind.config.ts');
  assert.match(tw, /breathe:/);
  const art = await read('../components/signature-art.tsx');
  assert.match(art, /animate-breathe/);
});

test('contact CTA uses a real target (tel or mailto)', async () => {
  const booking = await read('../components/booking.tsx');
  assert.match(booking, /href="(tel|mailto):/);
  const visit = await read('../components/visit.tsx');
  assert.match(visit, /href="tel:/);
});
